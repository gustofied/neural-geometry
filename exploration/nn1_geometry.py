"""
nn1_geometry.py — ReLU geometry project

Radial-band dataset, 2-class softmax classifier. Shows training curves,
logit-difference radial probe, and geometry dashboard (boundary + regions).
"""
import numpy as np
...
def train(model, X, y, lr, nb_epoch, batch_size=64):
    Y   = np.eye(int(y.max()) + 1)[y]   # one-hot, derived here — caller only needs class ids
...
def get_joint_regions(model, grid):
    model.forward(grid)
    mask1      = model.layers[1].x_in > 0
    mask2      = model.layers[3].x_in > 0
    joint_mask = np.concatenate([mask1, mask2], axis=1)
...
def plot_radial_probe(model, n_radii=8, r_max=5.0, n_points=300):
    """Logit difference (class 1 - class 0) vs radius along n_radii directions.
    The geometric quantity: shows how the piecewise-linear score behaves radially."""
    ...
    for theta in angles:
        ...
        diff   = np.clip(logits[:, 1] - logits[:, 0], -12, 12)
        ax.plot(radii, diff, color=REGION_WIRE, linewidth=0.8, alpha=0.6)
    ...
    _clean_ax(ax, "LOGIT DIFFERENCE vs RADIUS")
...
def plot_geometry_dashboard(model, X, y, h=0.008):
...
plot_geometry_dashboard(net, X, y)
